...
fm_process = None
on_off = ["off", "on"]
config_location = "/etc/radio.config"
...
def read_config():
	...
	try:
		...
		repeat_all = config.getboolean("radio",'repeat_all', fallback=True)
		music_dir = config.get("radio", 'music_dir', fallback="/opt/radio/content")
...
def run_pifm(use_audio_in=False):
	global fm_process
	with open(os.devnull, "w") as dev_null:
		fm_process = subprocess.Popen(["/opt/radio/pifm","-",str(frequency),"44100", "stereo" if play_stereo else "mono"], stdin=music_pipe_r, stdout=dev_null)
