...
# Generate severall configuration files out of anyfestoweb.conf
...
#    HOSTNAME          =  anyfesto.lan   - o'rly?  Maybe generate some additional stuff here
#  -
#    GLOBAL_CHAT       = Enable Broadcasts
#    GLOBAL_DEST       = Broadcast IP destinations
#    PYTHONPATH        = Path of PirateBox python libs
#    ANYFESTOWEB         = Anyfestoweb Folder
#    
#
#  Matthias Strubel    -- 08.06.2012
#    licenced with GPL-3
# moded  for Anyfesto  Tom Higgins 5-2014
...
if [ -z  $1 ] ; then
  echo "Usage is 
      generate_config_files.sh /opt/anyfestoweb/conf/anyfestoweb.conf
   "
   exit 255
fi
