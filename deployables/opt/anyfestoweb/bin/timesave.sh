...
#    moded for Anyfesto tomhiggins

# Load configfile

if [ -z  $1 ] || [ -z $2 ] ; then
  echo "Set up a crontab entry for regulary saving the time"
  echo "Usage $0 <path to anyfestoweb.conf> <step>"
...
TIMESAVE="$ANYFESTO_FOLDER/timesave_file"

if [ "$2" = "install" ] ; then
    crontab -l   >  $ANYFESTO_FOLDER/tmp/crontab 2> /dev/null
    echo "#--- Crontab for Anyfesto-Timesave" >>  $ANYFESTO_FOLDER/tmp/crontab
    echo " */5 * * * *   $ANYFESTO_FOLDER/bin/timesave.sh $ANYFESTO_FOLDER/conf/anyfestoweb.conf save "  >> $ANYFESTO_FOLDER/tmp/crontab
    crontab $ANYFESTO_FOLDER/tmp/crontab
